  ...
  mediaDir: string,
    ...
    for (const [index, asset] of assets.entries()) {
      ...
      const args = isActualVideo
        ? ['-y', '-stream_loop', '1', '-i', source, '-t', String(sceneDuration), '-an', '-vf', filter, '-r', '30', '-threads', '1', scene]
        : ['-y', '-loop', '1', '-i', source, '-t', String(sceneDuration), '-an', '-vf', filter, '-r', '30', '-threads', '1', scene]
