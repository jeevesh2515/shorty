...
export class ShortsDatabase {
  ...
  private mapScript(row: Record<string, unknown>): Script { const rawTags = safeJsonParse(row.tags_json as string, []); const tagsSuggestion = Array.isArray(rawTags) ? rawTags.map(t => String(t).replace(/^#/, '').trim()).filter(Boolean) : typeof rawTags === 'string' ? (rawTags as string).split(/\s+/).map(t => t.replace(/^#/, '').trim()).filter(Boolean) : []; return { id: String(row.id), topicId: String(row.topic_id), text: String(row.text), durationSec: Number(row.duration_sec), hook: String(row.hook), cta: row.cta ? String(row.cta) : undefined, titleSuggestion: row.title_suggestion ? String(row.title_suggestion) : undefined, descriptionSuggestion: row.description_suggestion ? String(row.description_suggestion) : undefined, tagsSuggestion, status: row.status as Script['status'], judgeScore: row.judge_score !== null && row.judge_score !== undefined ? Number(row.judge_score) : undefined, judgeVerdict: row.judge_verdict ? (row.judge_verdict as Script['judgeVerdict']) : undefined, judgeFeedback: row.judge_feedback ? String(row.judge_feedback) : undefined, judgeCriteria: safeJsonParse(row.judge_criteria_json as string, undefined), createdAt: String(row.created_at), updatedAt: String(row.updated_at) } }
