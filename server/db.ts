...
export class ShortsDatabase {
  ...
  private migrate() {
    ...
    if (!migration) {
      ...
      CREATE TABLE IF NOT EXISTS videos (
        ...
        thumbnail_url TEXT, status TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL
      );
  ...
  createScript(script: Script) {
    ...
    this.db.prepare(`INSERT INTO scripts (id,topic_id,text,duration_sec,hook,cta,title_suggestion,description_suggestion,tags_json,status,created_at,updated_at) VALUES (@id,@topicId,@text,@durationSec,@hook,@cta,@titleSuggestion,@descriptionSuggestion,@tags,@status,@createdAt,@updatedAt)`).run({ ...script, cta: script.cta || null, titleSuggestion: script.titleSuggestion || null, descriptionSuggestion: script.descriptionSuggestion || null, tags: JSON.stringify(script.tagsSuggestion) })
    this.audit('script', script.id, 'created', script.status, 'Script created', { topicId: script.topicId })
    return script
  }
  ...
  updateVideo(id: string, patch: Partial<Pick<Video, 'status' | 'audioUrl' | 'visualAssets' | 'finalVideoUrl' | 'thumbnailUrl'>>) { const current = this.getVideo(id); if (!current) return undefined; if (patch.status) assertStatusTransition('video', current.status, patch.status); const next = { ...current, ...patch, updatedAt: nowIso() }; this.db.prepare(`UPDATE videos SET audio_url=@audioUrl,visual_assets_json=@visualAssets,final_video_url=@finalVideoUrl,thumbnail_url=@thumbnailUrl,status=@status,updated_at=@updatedAt WHERE id=@id`).run({ id, audioUrl: next.audioUrl || null, visualAssets: JSON.stringify(next.visualAssets), finalVideoUrl: next.finalVideoUrl || null, thumbnailUrl: next.thumbnailUrl || null, status: next.status, updatedAt: next.updatedAt }); if (patch.status) this.audit('video', id, 'status_changed', patch.status, `Video status changed to ${patch.status}`, { from: current.status }); return this.getVideo(id) }
