...
export class ShortsDatabase {
  ...
  private migrate() {
    ...
    if (!migration) {
      ...
      CREATE TABLE IF NOT EXISTS scripts (
        ...
        factual_sources_json TEXT NOT NULL DEFAULT '[]'
      );
    ...
    const sourcesMigration = this.db.prepare('SELECT version FROM schema_migrations WHERE version = 4').get() as { version: number } | undefined
    if (!sourcesMigration) {
      try { this.db.exec("ALTER TABLE scripts ADD COLUMN factual_sources_json TEXT NOT NULL DEFAULT '[]'") } catch (error) { if (!(error instanceof Error) || !error.message.includes('duplicate column')) throw error }
      this.db.prepare('INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (4, ?)').run(nowIso())
    }
  }
  ...
  createScript(script: Script) {
    ...
    this.db.prepare('DELETE FROM scripts WHERE topic_id = ?').run(script.topicId)
    this.db.prepare(`INSERT INTO scripts (id,topic_id,text,duration_sec,hook,cta,title_suggestion,description_suggestion,tags_json,status,judge_score,judge_verdict,judge_feedback,judge_criteria_json,factual_sources_json,created_at,updated_at) VALUES (@id,@topicId,@text,@durationSec,@hook,@cta,@titleSuggestion,@descriptionSuggestion,@tags,@status,@judgeScore,@judgeVerdict,@judgeFeedback,@judgeCriteria,@factualSources,@createdAt,@updatedAt)`).run({ ...script, cta: script.cta || null, titleSuggestion: script.titleSuggestion || null, descriptionSuggestion: script.descriptionSuggestion || null, tags: JSON.stringify(script.tagsSuggestion), judgeScore: script.judgeScore ?? null, judgeVerdict: script.judgeVerdict || null, judgeFeedback: script.judgeFeedback || null, judgeCriteria: JSON.stringify(script.judgeCriteria || {}), factualSources: JSON.stringify(script.factualSources || []) })
    this.audit('script', script.id, 'created', script.status, 'Script created', { topicId: script.topicId, judgeScore: script.judgeScore })
    return script
  }
  listScripts(): Script[] { return (this.db.prepare('SELECT * FROM scripts ORDER BY created_at DESC, rowid DESC').all() as Record<string, unknown>[]).map(row => this.mapScript(row)) }
  ...
  getScriptForTopic(topicId: string) { const row = this.db.prepare('SELECT * FROM scripts WHERE topic_id = ? ORDER BY created_at DESC, rowid DESC LIMIT 1').get(topicId) as Record<string, unknown> | undefined; return row ? this.mapScript(row) : undefined }
  ...
  importState(state: { topics?: Topic[]; scripts?: Script[]; videos?: Video[]; uploads?: Upload[]; analytics?: Analytics[] }) {
    const tx = this.db.transaction(() => {
      if (Array.isArray(state.topics)) {
        const stmt = this.db.prepare(`INSERT INTO topics (id,title,niche,source,status,metrics_json,rationale,created_at,updated_at) VALUES (@id,@title,@niche,@source,@status,@metrics,@rationale,@createdAt,@updatedAt) ON CONFLICT(id) DO UPDATE SET title=excluded.title, niche=excluded.niche, source=excluded.source, status=excluded.status, metrics_json=excluded.metrics_json, rationale=excluded.rationale, updated_at=excluded.updated_at`)
        for (const item of state.topics) {
          stmt.run({ ...item, rationale: item.rationale || null, metrics: JSON.stringify(item.metrics || {}), updatedAt: item.updatedAt || item.createdAt || nowIso() })
        }
      }
      if (Array.isArray(state.scripts)) {
        const stmt = this.db.prepare(`INSERT INTO scripts (id,topic_id,text,duration_sec,hook,cta,title_suggestion,description_suggestion,tags_json,status,judge_score,judge_verdict,judge_feedback,judge_criteria_json,factual_sources_json,created_at,updated_at) VALUES (@id,@topicId,@text,@durationSec,@hook,@cta,@titleSuggestion,@descriptionSuggestion,@tags,@status,@judgeScore,@judgeVerdict,@judgeFeedback,@judgeCriteria,@factualSources,@createdAt,@updatedAt) ON CONFLICT(id) DO UPDATE SET text=excluded.text, duration_sec=excluded.duration_sec, hook=excluded.hook, cta=excluded.cta, title_suggestion=excluded.title_suggestion, description_suggestion=excluded.description_suggestion, tags_json=excluded.tags_json, status=excluded.status, judge_score=excluded.judge_score, judge_verdict=excluded.judge_verdict, judge_feedback=excluded.judge_feedback, judge_criteria_json=excluded.judge_criteria_json, factual_sources_json=excluded.factual_sources_json, updated_at=excluded.updated_at`)
        for (const item of state.scripts) {
          stmt.run({ ...item, cta: item.cta || null, titleSuggestion: item.titleSuggestion || null, descriptionSuggestion: item.descriptionSuggestion || null, tags: JSON.stringify(item.tagsSuggestion || []), judgeScore: item.judgeScore ?? null, judgeVerdict: item.judgeVerdict || null, judgeFeedback: item.judgeFeedback || null, judgeCriteria: JSON.stringify(item.judgeCriteria || {}), factualSources: JSON.stringify(item.factualSources || []), updatedAt: item.updatedAt || item.createdAt || nowIso() })
        }
      }
      if (Array.isArray(state.videos)) {
        const stmt = this.db.prepare(`INSERT INTO videos (id,script_id,audio_url,visual_assets_json,final_video_url,thumbnail_url,render_manifest_json,status,created_at,updated_at) VALUES (@id,@scriptId,@audioUrl,@visualAssets,@finalVideoUrl,@thumbnailUrl,@renderManifest,@status,@createdAt,@updatedAt) ON CONFLICT(id) DO UPDATE SET audio_url=excluded.audio_url, visual_assets_json=excluded.visual_assets_json, final_video_url=excluded.final_video_url, thumbnail_url=excluded.thumbnail_url, render_manifest_json=excluded.render_manifest_json, status=excluded.status, updated_at=excluded.updated_at`)
        for (const item of state.videos) {
          stmt.run({ ...item, audioUrl: item.audioUrl || null, visualAssets: JSON.stringify(item.visualAssets || []), finalVideoUrl: item.finalVideoUrl || null, thumbnailUrl: item.thumbnailUrl || null, renderManifest: JSON.stringify(item.renderManifest || {}), updatedAt: item.updatedAt || item.createdAt || nowIso() })
        }
      }
      if (Array.isArray(state.uploads)) {
        const stmt = this.db.prepare(`INSERT INTO uploads (id,video_id,youtube_video_id,youtube_url,title,description,tags_json,thumbnail_url,scheduled_at,status,idempotency_key,created_at,updated_at) VALUES (@id,@videoId,@youtubeVideoId,@youtubeUrl,@title,@description,@tags,@thumbnailUrl,@scheduledAt,@status,@idempotencyKey,@createdAt,@updatedAt) ON CONFLICT(id) DO UPDATE SET youtube_video_id=excluded.youtube_video_id, youtube_url=excluded.youtube_url, title=excluded.title, description=excluded.description, tags_json=excluded.tags_json, thumbnail_url=excluded.thumbnail_url, scheduled_at=excluded.scheduled_at, status=excluded.status, updated_at=excluded.updated_at`)
        for (const item of state.uploads) {
          stmt.run({ ...item, youtubeVideoId: item.youtubeVideoId || null, youtubeUrl: item.youtubeUrl || null, description: item.description || null, tags: JSON.stringify(item.tags || []), thumbnailUrl: item.thumbnailUrl || null, scheduledAt: item.scheduledAt || null, updatedAt: item.updatedAt || item.createdAt || nowIso() })
        }
      }
      if (Array.isArray(state.analytics)) {
        const stmt = this.db.prepare(`INSERT INTO analytics (id,upload_id,views,average_view_duration_sec,swipe_away_rate,likes,comments,subscribers_gained,estimated_revenue,fetched_at) VALUES (@id,@uploadId,@views,@averageViewDurationSec,@swipeAwayRate,@likes,@comments,@subscribersGained,@estimatedRevenue,@fetchedAt) ON CONFLICT(id) DO UPDATE SET views=excluded.views, average_view_duration_sec=excluded.average_view_duration_sec, swipe_away_rate=excluded.swipe_away_rate, likes=excluded.likes, comments=excluded.comments, subscribers_gained=excluded.subscribers_gained, estimated_revenue=excluded.estimated_revenue, fetched_at=excluded.fetched_at`)
        for (const item of state.analytics) {
          stmt.run(item)
        }
      }
    })
  ...
  private mapScript(row: Record<string, unknown>): Script { const rawTags = safeJsonParse(row.tags_json as string, []); const tagsSuggestion = Array.isArray(rawTags) ? rawTags.map(t => String(t).replace(/^#/, '').trim()).filter(Boolean) : typeof rawTags === 'string' ? (rawTags as string).split(/\s+/).map(t => t.replace(/^#/, '').trim()).filter(Boolean) : []; const rawSources = safeJsonParse<unknown[]>(row.factual_sources_json as string, []); const factualSources = Array.isArray(rawSources) ? rawSources.map(s => String(s)).filter(s => /^https?:\/\//.test(s)) : []; return { id: String(row.id), topicId: String(row.topic_id), text: String(row.text), durationSec: Number(row.duration_sec), hook: String(row.hook), cta: row.cta ? String(row.cta) : undefined, titleSuggestion: row.title_suggestion ? String(row.title_suggestion) : undefined, descriptionSuggestion: row.description_suggestion ? String(row.description_suggestion) : undefined, tagsSuggestion, factualSources: factualSources.length ? factualSources : undefined, status: row.status as Script['status'], judgeScore: row.judge_score !== null && row.judge_score !== undefined ? Number(row.judge_score) : undefined, judgeVerdict: row.judge_verdict ? (row.judge_verdict as Script['judgeVerdict']) : undefined, judgeFeedback: row.judge_feedback ? String(row.judge_feedback) : undefined, judgeCriteria: safeJsonParse(row.judge_criteria_json as string, undefined), createdAt: String(row.created_at), updatedAt: String(row.updated_at) } }
