...
const POSTHOG_CONFIGURED = Boolean(import.meta.env.VITE_POSTHOG_KEY && import.meta.env.VITE_POSTHOG_HOST)
const capture: typeof posthog.capture = (...args) => {
  if (POSTHOG_CONFIGURED) posthog.capture(...args)
}
...
function App() {
  ...
  const runManualShort = async () => {
    if (API_MODE) {
      try {
        ...
        capture('manual_short_run_completed', { execution_mode: 'api' })
    ...
    capture('manual_short_run_completed', { execution_mode: 'offline' })
  ...
  const generateScript = async (topic: Topic) => {
    if (API_MODE) {
      try {
        ...
        await refreshFromApi()
        capture('script_generation_completed', { execution_mode: 'api' })
    ...
    if (existing) {
      ...
      capture('script_generation_completed', { execution_mode: 'offline', reused_existing_script: true })
    ...
    capture('script_generation_completed', { execution_mode: 'offline', reused_existing_script: false })
  ...
  const produceVideoFromScript = async (scriptId: string) => {
    if (API_MODE) {
      try {
        ...
        await refreshFromApi()
        capture('video_production_completed', { execution_mode: 'api' })
    ...
    capture('video_production_completed', { execution_mode: 'offline' })
    showToast('Video produced and added to queue')
  }

  const rejectTopic = async (topicId: string) => { if (API_MODE) { try { await apiRequest(`/api/topics/${topicId}/status`, { method: 'PATCH', body: JSON.stringify({ status: 'rejected' }) }); await refreshFromApi(); capture('topic_rejected', { execution_mode: 'api' }); showToast('Topic rejected and removed from the active queue') } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to reject topic') }; return } updateState(current => ({ ...current, topics: current.topics.map(topic => topic.id === topicId ? { ...topic, status: 'rejected' } : topic) })); capture('topic_rejected', { execution_mode: 'offline' }); showToast('Topic rejected and removed from the active queue') }
  const rerenderVideo = async (videoId: string) => { if (API_MODE) { try { await apiRequest(`/api/videos/${videoId}/render`, { method: 'POST' }); await refreshFromApi(); capture('video_rerender_completed', { execution_mode: 'api' }); showToast('Video rendered successfully with the configured media pipeline') } catch (error) { showToast(error instanceof Error ? error.message : 'Video render failed') }; return } updateState(current => ({ ...current, videos: current.videos.map(video => video.id === videoId ? { ...video, status: 'ready', finalVideoUrl: video.finalVideoUrl || 'https://cdn.coverr.co/videos/coverr-aerial-view-of-a-mountain-road-1576/1080p.mp4' } : video) })); capture('video_rerender_completed', { execution_mode: 'offline' }); showToast('Render retried successfully — video is ready') }
  const resyncAnalytics = async (uploadId: string) => { if (API_MODE) { try { await apiRequest('/api/analytics/sync', { method: 'POST' }); await refreshFromApi(); capture('analytics_sync_completed', { execution_mode: 'api' }); showToast('Analytics synced from YouTube') } catch (error) { showToast(error instanceof Error ? error.message : 'Analytics sync failed') }; return } updateState(current => ({ ...current, analytics: current.analytics.map(item => item.uploadId === uploadId ? { ...item, views: item.views + (item.views ? Math.round(item.views * 0.06) : 4200), likes: item.likes + (item.likes ? Math.round(item.likes * 0.04) : 230), subscribersGained: item.subscribersGained + (item.subscribersGained ? 19 : 28), fetchedAt: new Date().toISOString() } : item) })); capture('analytics_sync_completed', { execution_mode: 'offline' }); showToast('Analytics synced just now') }
  ...
  const approveForPublish = async (uploadId: string) => { if (API_MODE) { try { await apiRequest(`/api/uploads/${uploadId}/approve`, { method: 'POST' }); await refreshFromApi(); capture('upload_approved_for_publish', { execution_mode: 'api' }); showToast('Approved for 18:00 Europe/London tomorrow') } catch (error) { showToast(error instanceof Error ? error.message : 'Approval failed') }; return } updateState(current => ({ ...current, uploads: current.uploads.map(upload => upload.id === uploadId ? { ...upload, status: 'approved_for_publish', scheduledAt: daysFromNow(1, 18) } : upload) })); capture('upload_approved_for_publish', { execution_mode: 'offline' }); showToast('Approved for 18:00 tomorrow') }
  const toggleAutomation = async () => { const next = !state.automationPaused; if (API_MODE) { try { await apiRequest('/api/settings/automation', { method: 'PATCH', body: JSON.stringify({ paused: next }) }); await refreshFromApi(); capture('automation_toggled', { automation_paused: next, execution_mode: 'api' }); showToast(next ? 'Autopilot paused' : 'Autopilot resumed') } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to update automation') }; return } updateState(current => ({ ...current, automationPaused: next })); capture('automation_toggled', { automation_paused: next, execution_mode: 'offline' }); showToast(next ? 'Autopilot paused' : 'Autopilot resumed') }
  const connectYouTube = async () => { if (!API_MODE) return; try { const redirect = await apiRequest<{ url: string }>('/api/auth/youtube'); capture('youtube_connection_started'); window.location.href = redirect.url } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to start YouTube OAuth') } }
  const disconnectYouTube = async () => { if (!API_MODE) return; try { await apiRequest('/api/auth/youtube/disconnect', { method: 'POST' }); await refreshFromApi(); capture('youtube_disconnected'); showToast('YouTube disconnected') } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to disconnect YouTube') } }
  ...
  const createTopic = async () => {
    if (API_MODE) { try { await apiRequest('/api/topics', { method: 'POST', body: JSON.stringify({ title: 'A new idea from the operator queue', niche: 'Productivity', source: 'manual', rationale: 'Added manually for the next content review.', metrics: { trendScore: 0, searchLift: 0, competition: 'Unscored' } }) }); await refreshFromApi(); capture('topic_created', { execution_mode: 'api' }); showToast('New topic added to the idea queue') } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to add topic') }; return }
    ...
    capture('topic_created', { execution_mode: 'offline' })
  ...
  const discoverTopics = async (niche = 'Science') => {
    if (API_MODE) {
      try {
        ...
        await refreshFromApi()
        capture('topic_discovery_completed', { execution_mode: 'api' })
    ...
    updateState(current => ({ ...current, topics: [topic, ...current.topics] }))
    capture('topic_discovery_completed', { execution_mode: 'offline' })
  ...
  const evaluateScriptJudge = async (scriptId: string) => {
    if (API_MODE) {
      try {
        ...
        await refreshFromApi()
        capture('script_judge_evaluated', { execution_mode: 'api' })
    ...
    capture('script_judge_evaluated', { execution_mode: 'offline' })
