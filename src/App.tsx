...
function App() {
  ...
  const navigate = (next: View) => { setView(next); setSelectedId(null); setMobileNavOpen(false) }
  ...
  const renderPage = () => {
    if (view === 'dashboard') return <DashboardPage state={state} nextUpload={nextUpload} lastPublished={lastPublished} publishedCount={publishedUploads.length} totalViews={totalViews} avgDuration={avgDuration} subs={subs} failedVideos={failedVideos} failedUploads={failedUploads} scriptsById={scriptsById} videosById={videosById} onRun={runManualShort} onNavigate={navigate} onOpen={openDetail} />
    if (view === 'topics') return <TopicsPage topics={state.topics} scriptsByTopic={scriptsByTopic} filter={topicFilter} niche={topicNiche} onFilter={setTopicFilter} onNiche={setTopicNiche} onOpen={openDetail} onGenerate={generateScript} onReject={rejectTopic} />
...
function Sidebar({ view, open, automationPaused, onNavigate, onClose }: { view: View; open: boolean; automationPaused: boolean; onNavigate: (view: View) => void; onClose: () => void }) {
  ...
  return <aside className={`sidebar ${open ? 'sidebar-open' : ''}`}>
    ...
    <div className={`automation-card ${automationPaused ? 'is-paused' : ''}`}><div className="automation-card-head"><span className="live-pulse" />{automationPaused ? 'Automation paused' : 'Autopilot is active'}</div><p>{automationPaused ? 'Your next Short will wait until you resume the queue.' : 'Next run checks the queue every morning at 09:00.'}</p><div className="automation-progress"><span style={{ width: automationPaused ? '22%' : '68%' }} /></div><small>{automationPaused ? 'Paused by operator' : 'Daily cadence · 1 Short / day'}</small></div>
...
function DashboardPage({ state, nextUpload, lastPublished, publishedCount, totalViews, avgDuration, subs, failedVideos, failedUploads, scriptsById, videosById, onRun, onNavigate, onOpen }: { state: AppState; nextUpload?: Upload; lastPublished?: Upload; publishedCount: number; totalViews: number; avgDuration: number; subs: number; failedVideos: Video[]; failedUploads: Upload[]; scriptsById: Map<string, Script>; videosById: Map<string, Video>; onRun: () => void; onNavigate: (view: View) => void; onOpen: (id: string) => void }) {
  return <>
...
function TopicsPage({ topics, scriptsByTopic, filter, niche, onFilter, onNiche, onOpen, onGenerate, onReject }: { topics: Topic[]; scriptsByTopic: Map<string, Script>; filter: string; niche: string; onFilter: (value: string) => void; onNiche: (value: string) => void; onOpen: (id: string) => void; onGenerate: (topic: Topic) => void; onReject: (id: string) => void }) {
